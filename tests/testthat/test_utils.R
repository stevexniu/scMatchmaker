...
test_that("Zero one normalization", {
  test.data <- 1:10
  test.res <- scMatchmaker:::zeroOne(test.data)
...
test_that("Filter matrix", {
  test.res <- scMatchmaker:::filterMatrix(matrix(1:9,3),low.cutoff = 3,high.cutoff = 6)
...
test_that("Aggregate by name", {
  ...
  test.res <- scMatchmaker:::aggregateName(mat, aggregate = "row")
  expect_equal(rownames(test.res), c(1:3,letters[1:3]))
  expect_equal(colnames(test.res), c(1:3,letters[1:3]))
  test.res <- scMatchmaker:::aggregateName(mat, aggregate = "col", split.by = "-")
  expect_equal(rownames(test.res), c(1:3,LETTERS[1:3]))
  expect_equal(colnames(test.res), c(1:3,LETTERS[1:3]))
  test.res <- scMatchmaker:::aggregateName(mat, aggregate = "row", cutoff = 10)
  expect_true(all(test.res == 0))
  test.res <- scMatchmaker:::aggregateName(mat, aggregate = "col", split.by = "-", counted = TRUE)
...
test_that("Random identity generation", {
  test.res <- scMatchmaker:::randomIdents(rep(1:2, each = 10), n = 1, seed = 123)
  set.seed(123)
